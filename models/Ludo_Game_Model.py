...
class Ludo_Game:
    ...
    def initialize(self, number_of_players): # Initialize function for setting up everything in the game
        ...
        self.ludo_table = np.zeros(COMMON_TABLE_SIZE, dtype=int)
        self.last_cells = np.array([np.zeros(LAST_CELLS, dtype=int) for i in range(number_of_players)])
        self.dice = 0
    ...
    def can_Enter_Last_Cells(self, last_common_cell, current_location, target_location):
        print("Can enter last cells: ", (current_location <= last_common_cell) & (target_location > last_common_cell))
    ...
    def display_Ludo_Table(self):
        print(self.ludo_table)

    def roll_Dice(self):
        value = np.random.randint(1, 9)
        if value > 6:
            value = 6 # Dice number has to be from 1-6
        self.dice = value
    ...
    def get_Pawn_State(self, player_number, pawn_number):
        ...
        if location >= 0: # If location is positive then the pawn is still in the common table
            ...
            value = 2**(6 + location) # location would be negative
        ...
        if current_location >= 0: # Pawn is still on common table
            ...
            if self.can_Enter_Last_Cells(self.players[player_number].last_common_cell, current_location, (current_location + self.dice)):
                ...
                if self.ludo_table[new_location] > 0 & self.unavailable_eating_cells.__contains__(new_location) is not True:
                    print(f"{player_number + 1} is eating a player")
                    eaten_player = self.ludo_table[new_location] - 1
                    print(f"Player eaten {eaten_player + 1} in {new_location}")
                    consecuence = (eaten_player, new_location)
                    self.pawn_Has_Been_Eaten(eaten_player, new_location)
            ...
            if self.players[player_number].pawns_in_table.__contains__(current_location) is not True:
